...
export function leavesData(
  ...
  view.setBigUint64(33, amount, true); // true = little-endian
...
async function GenMerkleTree(
  ...
  const userAmountsBN: Record<string, userDataBO> = Object.fromEntries(
    Object.entries(userData).map(([key, value]) => {
      const lamports = new Decimal(value).mul(
  ...
  const userProofs = Object.fromEntries(
    Object.entries(userAmountsBN).map(([address, data]) => {
      const proof = tree
